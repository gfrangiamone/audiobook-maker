...
| D6 | Gate di qualità in due stadi: misure lato server (porting di `tools/voice_prompts/audio.py` del worker, numpy) più **verifica ASR lato worker** (whisper, CER contro la frase guidata) come anti-impersonificazione. |
...
| D14 | Revoca di un dispositivo e cancellazione della voce: **link firmati** nell'email del proprietario. L'email invita a conservarla. |
...
| D16 | Dopo il pagamento il server è **autonomo**: genera le demo anche se il browser sparisce. L'approvazione è un passo UX, non un prerequisito per rendere la voce usabile. |
| D17 | L'email inviata al pagamento è **lo strumento di ripresa** del wizard: contiene il codice-voce e un link firmato che riapre il wizard al punto giusto. |
...
onesto sull'attesa: «da uno a quattro minuti» (cold start del worker). Al
...
più tardi» (non conta come rigenerazione) e spiega che la voce è comunque
utilizzabile.
...
### 5.4 Verifica ASR lato worker

Nuova action `check_sample` nel repo `abm-voxcpm-worker`
(`handler.py`), che riusa `verifica.py` (faster-whisper `base`, già a bordo):

```
input:  {"action": "check_sample", "wav_b64": ..., "language": "it",
         "expected_text": "<frase guidata>"}
output: {"cer": 0.07, "heard": "...", "seconds": 1.9}
```

Soglia `ABM_VOICE_CLONE_MAX_CER` (default 0.25, calibrata su prove reali
prima del rilascio). Sopra soglia il gate fallisce con `vc_gate_text`.
La chiamata avviene dopo il gate locale (non si paga GPU per un file
rumoroso) e prima del riascolto. Costa un cold start se il worker è freddo:
il pannello mostra «verifica in corso, fino a tre minuti». Fallimento del
worker (timeout, rimbalzo): la verifica viene **rinviata** al momento del
commit, il campione passa in `sample_ok` con `asr_pending=true`, e il
commit la ripete prima di consumare il pagamento; se fallisce lì, il
pagamento non viene consumato e l'utente rifà il campione.

Questa action richiede build e New Release manuale dell'endpoint RunPod: è
un prerequisito di deploy, non di sviluppo (test con mock).
...
- `id` pubblico (`vc_...`): usato negli endpoint del wizard, senza potere
  da solo (ogni endpoint verifica il cid o un link firmato).
...
`via: resume` dal link firmato dell'email di pagamento (§8.1): quel link è
del proprietario, quindi vale come conferma.

Rimozione dal dispositivo: `POST /api/voice_clone/<id>/forget` (cid
corrente). Revoca da parte del proprietario: link firmato nell'email (§8.3),
pagina `/vc/<sig>/devices` con l'elenco (data, via, ultimi 4 caratteri del
cid) e un bottone di revoca per riga.
...
Link firmato nell'email (§8.3) → pagina di conferma → `state = deleted`,
...
Link firmati: `base64url(payload) + "." + hmac_sha256(ABM_VOICE_CLONE_SIGN_KEY,
payload)[:32]`, payload = `{"id", "action", "exp"}` (`exp` assente per i link
senza scadenza). Il progetto non ha una secret key condivisa e il token admin
può ruotare senza dover invalidare i link degli utenti: per questo la chiave
è dedicata (`ABM_VOICE_CLONE_SIGN_KEY`, obbligatoria se la feature è attiva).
Un link firmato è invalido anche se la voce è `deleted`, `refunded` o
`expired`.

### 8.1 Pagamento ricevuto (subito dopo il commit)

Oggetto «La tua voce campione: codice e ricevuta». Contiene: importo,
codice-voce in grande, «conserva questa email», link firmato **Riprendi la
...
(entrambi firmati, senza scadenza ma revocabili cancellando la voce),
...
| `/api/generate`, `/api/preview`, stime | se la voce è `mine`: 403 `voice_not_authorized` se il cid non è in `devices`; 400 `voice_lang_mismatch` se lingua/accento del libro non coincidono; 410 `voice_gone` se lo stato non è `demos_ready`, `demo_failed` o `ready` (D16: la voce è usabile dal momento in cui il campione è pagato e le demo sono state tentate). Poi `last_used_at` aggiornato. |
...
| Worker giù / rimbalzi / cold start lento | retry con backoff come `synthesize_chapter`; dopo `ABM_VOICE_CLONE_DEMO_RETRIES` (3) cicli → `demo_failed`, email «riprova più tardi» all'utente, notifica admin. Nessun rimborso automatico: la voce è già utilizzabile per i libri (il campione c'è). Il «Riprova» dell'utente non consuma rigenerazioni. |
...
| Approvazione mai data | la voce resta `demos_ready`, utilizzabile comunque dalla combo (D16) con le ultime demo generate come player; l'email finale parte dopo 24 ore in `demos_ready` senza azione, con nota «puoi ancora rigenerare le demo». |
...
- Nessun endpoint accetta il token: solo `id` pubblico più cid, o link
  firmati. Il `voice_code` da solo non dà accesso senza conferma.
...
| `ABM_VOICE_CLONE_ASR` | `1` | `0` salta la verifica ASR (sviluppo senza worker) |
| `ABM_VOICE_CLONE_REGEN_MAX` | `3` | rigenerazioni demo incluse |
| `ABM_VOICE_CLONE_DEMO_RETRIES` | `3` | cicli di retry prima di `demo_failed` |
| `ABM_VOICE_CLONE_SIGN_KEY` | vuoto | chiave HMAC dei link firmati; obbligatoria se la feature è attiva |
...
| `voice_clone_audio.py` | conversione, misure, gate (§5.1-5.3) |
| `voice_clone.py` | store, macchina a stati, codici, dispositivi, link firmati, demo thread, recovery, retention (§6, §10, §11) |
...
| `abm-voxcpm-worker/handler.py` | action `check_sample` |
| `requirements.txt` | `numpy` esplicito |
...
- `test_voice_clone.py`: macchina a stati completa; commit idempotente;
  ...
  link firmati (validi, scaduti, manomessi); retention e avviso; cancellazione;
...
- `test_voxcpm_tts.py`: `clone_block` con id `mine`, `check_sample` con mock.
...
- Soglia CER da calibrare su registrazioni reali nelle nove lingue prima del
  rilascio; whisper `base` su hindi e cinese va verificato.
