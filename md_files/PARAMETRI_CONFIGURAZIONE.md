...
| `ABM_SUPPORT_EMAIL` | `"support@audiobook-maker.com"` (destinatario del form "Contatta supporto" nel footer; casella inoltrata via MX esterno alla mailbox del titolare). L'email di assistenza mostra come mittente `utente@x.it (via Audiobook Maker) <ABM_SMTP_FROM>`: l'email dell'utente sta nel **nome visualizzato**, l'indirizzo resta il nostro perche' l'inoltro MX di questa casella rompe SPF e un `From` di dominio terzo farebbe finire il messaggio in spam (o rifiutare, con DMARC `p=reject` del mittente) | `email_service.py` | 31 |
...
| `ABM_QUOTE_LOCK_TTL_SEC` | `1800` (30 min; validità del **quote lock**: l'importo mostrato da `/api/combined_estimate` resta il prezzo esigibile alla creazione dell'ordine PayPal e sui path voucher/`/api/generate`/`/api/optimize`. Copre il tratto *stima → pagamento*, dove il price lock (`ABM_PRICE_LOCK_TTL_SEC`) non arriva ancora: la media mobile empirica si muove anche solo perché l'utente ascolta un'anteprima voce fra il preventivo e il click su PayPal, e il ricalcolo live rispondeva `400 amount mismatch`. Registrato per firma degli input di prezzo (voce, capitoli, velocità, lingua, AI on/off), max 8 per job; se la firma cambia o il TTL scade il server risponde `409 price_changed` e il client ri-quota e aggiorna la modale. Incidente 30/08/2026, utente DE: `amount mismatch (server=24.58, client=24.1)` ricorrente dal commit `933caa6`, che ha raggruppato i campioni per (lingua, voce, velocità) rendendo il rate delle lingue meno frequentate molto più mobile) | `audiobook_app.py` | 907 |
...
- **Capture PENDING non finanziate (eCheck)**: una capture PayPal puo' tornare `status=PENDING`, cioè **denaro non ancora sul conto**. Poiché ABM eroga il servizio contestualmente al pagamento, i reason elencati in `ABM_PAYPAL_UNFUNDED_PENDING_REASONS` (default: `ECHECK` e affini) sollevano `payment.UnfundedCaptureError`: nessun `payment_token` emesso, HTTP 402 con `paypal_issue=UNFUNDED_PENDING` e `retryable=true` (il frontend riapre il checkout per un altro metodo). Il record resta in `_payments` con `pending_unfunded=True` — **non spendibile** come payment token, **non rimborsabile** in automatico e **non incassabile** da `settle_delivered_captures_for_job` (rimborsare o incassare denaro mai ricevuto sarebbe una seconda perdita) — visibile per la riconciliazione manuale. All'utente non viene mostrato il generico "rifiutato dalla banca" ma la chiave i18n dedicata `pay_paypal_unfunded` ("addebito bancario/eCheck non accettato: l'accredito richiede giorni e può essere annullato — paga con carta o saldo PayPal"). Incidente 2026-08: 4 eCheck respinti dalla banca del pagante dopo l'erogazione, €12,93 di servizio consegnato e mai incassato. Blocco complementare consigliato lato conto PayPal: *Impostazioni → Pagamenti → Preferenze di incasso → blocca pagamenti eCheck*.
- **Gate preventivo `IMMEDIATE_PAYMENT_REQUIRED`** (`ABM_PAYPAL_IMMEDIATE_PAYMENT`, default on): `_paypal_create_order` chiede a PayPal solo strumenti a compensazione immediata, così l'eCheck non è nemmeno selezionabile nel checkout e il caso sopra non si presenta. Il filtro sul `payment_status` alla capture resta attivo come rete di sicurezza (ordini creati prima del deploy, preferenza ignorata da PayPal per account/paese).
...
- **Guardia CLI «servizio acceso»**: l'app carica `_vouchers.json` in RAM **una sola volta all'avvio** e lo riscrive per intero a ogni `_save_vouchers()`. Una scrittura esterna a servizio acceso resta quindi invisibile al processo vivo (il voucher non compare in `/admin/vouchers`, viene **rifiutato** alla validazione del cliente) e viene **sovrascritta** al primo salvataggio dell'app. Per questo `create` e `revoke` della CLI rilevano il processo (`systemctl is-active` sull'unit, fallback `pgrep -f audiobook_app.py`) e **rifiutano** l'operazione con exit code `4`, rimandando alla UI web. Il rilevamento e' best-effort: se gli strumenti di sistema mancano (sviluppo su Windows) la scrittura passa. Override deliberato con `--force` (tracciato in `voucher_admin.log` come `FORCE`). Nome dell'unit systemd configurabile con `ABM_SERVICE_UNIT` (default `audiobook-maker.service`). `list` e `show` restano liberi: emettono solo una nota su stderr.
...
| `ABM_VOXCPM_FREE_MAX_CHARS` | `100000` Cap di caratteri del **libro intero** (non della selezione di capitoli) oltre il quale una voce VoxCPM non è mai gratuita: sotto soglia e con quota capiente si paga comunque il floor `ABM_VOXCPM_MIN_COST_EUR` (`decision()` risponde `free_cap_exceeded`, il 402 di `/api/generate` è il generico `payment_required`, non `free_quota_exhausted`; il flag viaggia come `free_cap_exceeded` sia nel 402 sia in `/api/combined_estimate`, e la UI ne ricava la nota `free_cap_note` — senza, l'utente vede lo stesso importo minimo su ogni selezione con il credito mensile ancora capiente e nessuna causa visibile, richiesta di assistenza del 21/09/2026). Nato con l'incidente del 21/09/2026: il worker GPU costa per accensione e un libro grande letto un capitolo per generazione resta un libro grande. Solo VoxCPM; `0` disattiva. Il conteggio usa `info.total_chars` (sopravvive allo spill dei testi), con fallback alla somma dei `char_count` dei capitoli. | `free_quota.py` | `_premium_free_max_chars` |
...
Il credito Cloudflare AI Gateway è **prepagato**, è denominato **in dollari** e non è leggibile via API (gli endpoint di fatturazione rispondono 403: il token è deliberatamente ristretto a Workers AI). Il saldo residuo è quindi una **stima**: `ABM_CF_CREDIT_BALANCE_USD` è il saldo che l'admin dichiara dopo ogni ricarica, da cui `tts_backend_state.credit_left_usd()` sottrae la spesa cumulata nel ledger locale (`add_spend()`, chiave globale `_credit` nel file di stato — il credito è unico per l'account, non per modello).

**Tutto il sottosistema ragiona in USD**, la valuta in cui Cloudflare denomina il credito e fattura Workers AI: la cifra del pannello e quella della dashboard del fornitore vanno confrontabili a occhio, senza un cambio di mezzo. Il costo per chiamata era già calcolato nativamente in USD (`_breakdown()` → `total_usd`), quindi il ledger in dollari **toglie** una conversione invece di aggiungerla. `ABM_GEMINI_USD_EUR_RATE` (§7.1) interviene solo per due cose: migrare un ledger vecchio scritto in euro e affiancare l'equivalente in euro nel pannello. **Non entra mai nel confronto che decide un allarme** — saldo, spesa e soglia si confrontano fra loro sempre in USD, così un ritocco del cambio non può far scattare né tacere il pre-allarme.

| Variabile | Default | Descrizione |
|-----------|---------|-------------|
| `ABM_CF_CREDIT_BALANCE_USD` | `0` | Saldo Cloudflare dichiarato dall'admin dopo l'ultima ricarica, **in USD**. `0` (non dichiarato) disabilita il pre-allarme: evita rumore costante su installazioni che non usano Cloudflare o non hanno ancora eseguito la procedura di accensione. |
| `ABM_CF_CREDIT_ALERT_USD` | `5` | Soglia di residuo stimato (USD) sotto cui scatta il pre-allarme sul credito. |
| `ABM_CF_CREDIT_CHECK` | `1` | Interruttore del controllo del credito. `0`/`false`/`no`/`off` spegne **pre-allarme e residuo**, lasciando intatta la contabilità (`add_spend` continua ad accumulare). Da usare quando la **ricarica automatica a soglia** è attiva sul pannello Cloudflare: lì il credito si rialza da solo, il residuo stimato non descrive più nulla di azionabile e l'email diventerebbe rumore periodico su una condizione che il fornitore risolve da sé. |

**`ABM_CF_CREDIT_CHECK=0` non è equivalente a `ABM_CF_CREDIT_BALANCE_USD=0`.** Entrambi producono silenzio, ma per ragioni diverse: il saldo a 0 dice «non so quanto credito ho», l'interruttore dice «non voglio che il credito venga sorvegliato». La differenza conta al primo incidente, davanti a un pannello muto: distingue una configurazione dimenticata da una scelta. A controllo spento il pannello mostra la **spesa cumulata** al posto del residuo, l'email di failover **non allega** il residuo, e `claim_credit_alert()` ritorna `False` **senza consumare** l'allarme — riaccendendo il controllo lo si ritrova ancora armato.

**Nomi precedenti.** `ABM_CF_CREDIT_BALANCE_EUR` / `ABM_CF_CREDIT_ALERT_EUR` sono ancora onorati come ripiego: il valore viene convertito al cambio dichiarato e usato lo stesso, con un avviso a stdout una volta per processo. Il ripiego non è cortesia — un deploy che arrivasse prima dell'aggiornamento dell'unit systemd leggerebbe `0`, e un saldo `0` spegne il pre-allarme in silenzio: l'admin scoprirebbe il credito finito dal failover, cioè proprio dall'evento che il pre-allarme esiste per anticipare. Allo stesso modo un ledger su disco con `spent_eur` viene riconvertito in USD al caricamento (mai buttato: ripartire da 0 speso su un credito già consumato farebbe risalire il residuo e tacere l'allarme fino all'esaurimento vero). La migrazione **non** tocca `_STATE_VERSION`, che governa le voci di modello: cambiarlo renderebbe illeggibile l'intero stato e spegnerebbe Cloudflare su tutti i modelli al primo avvio dopo il deploy.

**API del modulo:** `add_spend(model_key, usd)`, `reset_spend()` (da chiamare alla ricarica: azzera il ledger e riarma l'allarme), `credit_left_usd()`, `credit_spent_usd()` (spesa cumulata, l'unica metà del conto significativa a controllo spento), `credit_balance_usd()` / `declared_balance_usd()` (saldo dichiarato, pure: `0` = residuo non conoscibile), `credit_alert_threshold_usd()`, `credit_check_enabled()`, `usd_eur_rate()` / `to_eur()` (solo display), `credit_alert_pending()`, `claim_credit_alert()`, `mark_credit_alerted()`.

**Il riarmo dell'allarme deve restare raggiungibile senza failover.** `reset_spend()` è l'unica funzione che riporta `alerted` a `False`, e il ciclo normale del credito (residuo sotto soglia → email di pre-allarme → ricarica → topup) avviene con Cloudflare **ancora sano**, cioè senza alcun trip. Il suo unico innesco in console è quindi il pulsante **«Ho ricaricato il credito»** del pannello «Backend TTS» (`POST /admin/api/tts_backend {"action":"topup"}`), abilitato ogni volta che `ABM_GEMINI_BACKEND=cloudflare` e **indipendente da `tripped_at`**. Nella forma precedente era una casella accanto al pulsante di rientro, abilitato solo dopo un trip: l'allarme partiva una volta sola nella vita dell'installazione e il residuo mostrato in console restava sbagliato per sempre (saldo dichiarato rialzato, `spent_usd` che continuava ad accumulare dal ciclo precedente).

**Dove scatta davvero il pre-allarme.** Subito dopo ogni `add_spend()` riuscito, in `gemini_tts._maybe_alert_credit(model_key)` (chiamata da `synthesize()` solo quando il backend che ha eseguito è Cloudflare): è l'unico istante in cui il residuo stimato può essere sceso sotto soglia, e il backend è ancora sano — cioè c'è ancora tempo per ricaricare. La funzione verifica **prima** che un notifier sia registrato e **poi** chiama `claim_credit_alert()`, mai il contrario: senza notifier la `claim` brucerebbe in silenzio l'unica notifica disponibile. Il notifier è registrato in `audiobook_app` (`_on_cf_credit_alert` → `email_service.admin_notify_cf_credit_low`), email **dedicata** e distinta da quella di failover (`admin_notify_tts_backend_switch`), che invece annuncia un guasto già avvenuto. `claim_credit_alert()` resta invocata anche dal notifier di switch, ma **solo** per sopprimere l'email di pre-allarme separata quando lo switch la anticipa: il residuo mostrato nell'email di failover si legge sempre da `credit_left_usd()` (pura) quando c'è un saldo dichiarato e il controllo del credito è acceso, mai dall'esito della claim. Legarlo a quell'esito faceva sparire il numero proprio quando serve di più — credito già sotto soglia significa allarme già consumato, cioè claim `False`.
...
| Variabile | Default | Descrizione |
|-----------|---------|-------------|
| `ABM_CF_TRIP_FAILURES` | `3` | Fallimenti consecutivi dopo i quali un errore `retryable` / `rate_limited` fa scattare il breaker. Floor a 1; valore non numerico → default. Un successo azzera il contatore. Gli errori `backend_down` fanno scattare **subito**, ignorando la soglia; `fatal` e `content_rejected` non fanno scattare nulla (sono difetti della richiesta, non del backend). Col timeout a 65 s il budget di failover e' 3 x 65 s = ~195 s: allungamento **voluto** (ruling 21/09/2026, vedi `ABM_CF_TIMEOUT_MS`), perche' la soglia serve a distinguere il guasto dalla lentezza, non ad accorciare l'attesa dell'utente. Abbassarla per «far scattare prima il breaker» e' il difetto che ha prodotto gli episodi del 19 e del 21/09/2026. File: `gemini_tts.py:_cf_trip_failures` (376). |
| `ABM_CF_TIMEOUT_MS` | `65000` | Timeout HTTP (ms) delle call verso Workers AI. Floor a 1000; valore non numerico → default. Indipendente dai timeout Vertex (`ABM_GEMINI_HTTP_TIMEOUT_MS*`) perche' la latenza del gateway Cloudflare ha un profilo diverso. **Storia: `60000` fino al 14/09/2026, `25000` fino al 19/09/2026, `45000` fino al 21/09/2026.** I 60 s erano troppi (tre chunk moribondi tenevano fermo un job pagato per 2 min 7 s prima del failover); i 25 s troppo pochi (risposte **riuscite** fra 11,9 s e 22,5 s nel log dell'AI Gateway: 31 timeout in un giorno su un backend sano). Il 21/09/2026 la coda si e' spostata di nuovo — risposte riuscite fra 11,7 s e **44,6 s** — e 45 s tagliavano di nuovo sulle richieste sane: breaker scattato su `flash31` alle 13:07:58 e **sonda di rientro fallita «dopo 45.1s» su poche parole di sintesi**, cioe' latenza indipendente dalla lunghezza del testo. A 65 s il margine sul peggior caso sano osservato e' ~1,5x. **Ruling 21/09/2026:** in questo backend il costo viene prima dell'attesa (Cloudflare e' l'opzione economica, Vertex azzera il margine), quindi il timeout si tara perche' non tagli mai una risposta sana, non perche' faccia scattare presto il failover; l'allungamento del budget (vedi `ABM_CF_TRIP_FAILURES`) e' voluto. In produzione la stessa cifra e' nell'unit systemd. File: `gemini_tts.py:_cf_timeout_ms`. |
...
| `ABM_CF_PROBE_FIRST_SEC` | `900` | Attesa (s) fra lo scatto del breaker e la **prima** sonda. Floor 60. **Era 1800, dimezzato il 21/09/2026**: il prezzo della sonda si e' rivelato molto piu' basso di quanto l'intervallo originale presupponesse (`_CF_PROBE_TEXT` e' una parola, l'addebito una frazione di centesimo, e una sonda fallita per timeout non viene nemmeno registrata dal gateway), mentre ogni mezz'ora in piu' su Vertex e' margine azzerato su tutti i job PREMIUM di quella finestra. File: `gemini_tts.py:_cf_probe_first_sec` (463). |
| `ABM_CF_PROBE_MAX_SEC` | `10800` | Tetto (s) dell'intervallo fra sonde, raggiunto raddoppiando a ogni fallimento (15 min → 30 min → 1 h → 2 h → 3 h). Floor 60. Il tetto esiste perche' senza di esso un guasto lungo porterebbe l'intervallo a giorni, e il rientro arriverebbe molto dopo la riparazione. **Era 21600, dimezzato il 21/09/2026** insieme a `ABM_CF_PROBE_FIRST_SEC`: al tetto vecchio un guasto notturno poteva tenere spento il backend economico fino al mattino dopo pur essendo rientrato da ore; il tetto nuovo costa al massimo otto sonde al giorno. File: `gemini_tts.py:_cf_probe_max_sec` (481). |
| `ABM_CF_PROBE_TIMEOUT_MS` | `15000` (ma mai sotto `ABM_CF_TIMEOUT_MS`) | Timeout (ms) della sola sonda di rientro. Floor 1000 **e pavimento a `_cf_timeout_ms()`**: il valore configurato e' un tetto desiderato, non puo' rendere la sonda piu' severa della produzione. Con il default attuale la sonda usa quindi 65000 (in produzione `ABM_CF_PROBE_TIMEOUT_MS=65000` e' dichiarato anche esplicitamente nell'unit). **Perche' il pavimento:** il 19/09/2026 il breaker di `flash31` non rientrava perche' la sonda a 15 s bocciava un gateway che serviva le stesse richieste in 12–22 s; il raddoppio dell'intervallo (30 min → 6 h) trasformava quel falso negativo in ore di Vertex. Una sonda piu' stretta del traffico vero non misura il backend, misura se stessa. File: `gemini_tts.py:_cf_probe_timeout_ms`. |
...
- **Retry per chunk** (`tts_split.generate_chunk_pcm_speechify`, `max_retries=3`, backoff `2^n` s): ritenta gli errori di rete (`Read timed out`, `ConnectionError`) e l'esaurimento dei 3 tentativi HTTP interni di `speechify_tts.synthesize` (429/5xx). Un 4xx fatale (`SpeechifyFatalError`, es. SSML invalido) **non** viene ritentato. Solo dopo il fallimento di tutti i tentativi il chunk diventa 1 s di silenzio (a 48 kHz). Timeout HTTP per chiamata: `120` s.
- **Pre-sintesi** (`generation_engine._speechify_presynth`): tutti i chunk vengono sintetizzati in un pool da `min(PER_JOB, MAX)` worker prima dell'assemblaggio; `progress_current`/`progress_message` ("Synthesizing audio: chunk n/N") avanzano a ogni chunk completato e il contatore resta monotono durante l'assemblaggio.
...
## 18. Moderazione anti-abuso della quota voci standard (`abuse_watch.py`)
