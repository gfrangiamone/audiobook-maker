...
| `ABM_TTS_MIN_CHUNK_CHARS` / `MIN_CHUNK_CHARS` | `40` (soglia caratteri sotto la quale un chunk e' considerato **degenere**, cioe' non vale la pena inviarlo da solo al TTS: viene fuso con il vicino da `_merge_degenerate_chunks` senza mai violare i cap `max_chars`/`max_bytes`, con preferenza per la fusione in avanti e rivalutazione a cascata. Governa **solo la fusione**, mai il silenziamento: un chunk corto che resta isolato viene comunque sintetizzato) | `tts_split.py` | 48 |
| `_DEGENERATE_MAX_CHARS` | `120` (finestra oltre la quale nessun criterio degenere si applica piu', in entrambi i predicati. Sopra questa soglia il chunk porta informazione: una cronologia di sole date e' un elenco da leggere, non un'intestazione da saltare) | `tts_split.py` | 51 |
| `_is_degenerate_chunk` / `_is_unspeakable_fragment` | Due predicati **distinti e non intercambiabili**. Il primo governa la fusione (criteri: piu' corto di `MIN_CHUNK_CHARS`, oppure composto per almeno meta' di numerali entro `_DEGENERATE_MAX_CHARS`) — un falso positivo qui costa nulla, si fonde e basta. Il secondo governa il **silenziamento senza chiamata API** in `generate_chunk_pcm_gemini`: e' vero solo se il chunk, entro `_DEGENERATE_MAX_CHARS`, non contiene **nessun token parlabile** (nessuna lettera, o solo numerali arabi/romani **ben formati** via `_STRICT_NUMERAL_TOKEN_RE`). L'asimmetria e' voluta: un falso negativo lascia l'errore di moderazione `2017` che il codice gia' subisce, un falso positivo cancella testo dall'audiolibro dichiarando `success`. Percio' prosa corta (`A mia madre.`, `Fine.`), titoli maiuscoli fatti di lettere romane (`CIVIL`, `MILD`, `DVD`) ed elenchi di date lunghi raggiungono comunque il backend. Ogni chunk silenziato stampa `[gemini-tts] Chunk degenere silenziato senza chiamata API (reason=wordless_fragment) job=<id>: <testo troncato>` e ritorna `skipped_degenerate: True` con 0 token e costo zero | `tts_split.py` | 439 / 472 |
