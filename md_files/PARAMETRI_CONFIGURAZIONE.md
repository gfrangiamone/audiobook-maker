...
| `ABM_CF_TIMEOUT_MS` | `45000` | Timeout HTTP (ms) delle call verso Workers AI. Floor a 1000; valore non numerico → default. Indipendente dai timeout Vertex (`ABM_GEMINI_HTTP_TIMEOUT_MS*`) perche' la latenza del gateway Cloudflare ha un profilo diverso. **Era `60000` fino al 14/09/2026, poi `25000` fino al 19/09/2026.** I 60 s erano troppi: tre chunk moribondi tenevano fermo un job pagato per 2 min 7 s prima del failover. I 25 s erano troppo pochi, e la motivazione che li reggeva («un chunk sano risponde in pochi secondi») era falsa: il log dell'AI Gateway del 19/09/2026 mostra risposte **riuscite** fra 11,9 s e 22,5 s, quindi la soglia tagliava sulla coda delle richieste sane — 31 timeout in un giorno, breaker scattato su un backend funzionante e job premium dirottati su Vertex. A 45 s il margine sul peggior caso sano osservato e' doppio; a proteggere il tempo dell'utente pensa `ABM_CF_TRIP_FAILURES` (2 in produzione: ~92 s al failover, meno dei 127 s dell'episodio originario). File: `gemini_tts.py:_cf_timeout_ms`. |
