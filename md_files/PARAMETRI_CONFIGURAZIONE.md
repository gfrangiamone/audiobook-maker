...
| `ABM_VOXCPM_ENDPOINT_ID` | — Endpoint RunPod serverless del worker VoxCPM. Assente: il motore non compare fra i modelli. | `voxcpm_tts.py` | 52 |
| `ABM_VOXCPM_API_KEY` | — Chiave API RunPod. Assente: il motore non compare fra i modelli. | `voxcpm_tts.py` | 56 |
| `ABM_VOXCPM_RATE_EUR_PER_MCHAR` | — Tariffa di listino all'utente, EUR per milione di caratteri, fee di pagamento incluse. Assente o 0: il motore non compare (meglio nascosto che venduto a un prezzo non deciso). | `voxcpm_tts.py` | 60 |
| `ABM_VOXCPM_FREE_THRESHOLD_EUR` | `0.50` Sotto questo importo il job è gratuito, come per Gemini e Speechify. | `voxcpm_tts.py` | 69 |
...
| `ABM_VOXCPM_COST_USD_PER_MCHAR` | `0.91` Costo GPU misurato (RTX 4090, 2026-08-04). Alimenta solo l'audit del margine, mai il prezzo all'utente. | `voxcpm_tts.py` | 65 |
| `ABM_VOXCPM_CONCURRENCY` | `32` Chunk in volo dentro un singolo job del worker. | `voxcpm_tts.py` | 73 |
| `ABM_VOXCPM_QUEUE_TIMEOUT_S` | `900` Quanto si aspetta in coda un job VoxCPM prima di dichiarare l'endpoint saturo. Scaduto, il job si cancella e non si ritenta. | `voxcpm_tts.py` | 244 |
| `ABM_VOXCPM_JOB_TIMEOUT_S` | `1800` Quanto puo' durare l'esecuzione di un job VoxCPM. Scaduto, il job si cancella (RunPod fattura a secondi) e si ritenta. | `voxcpm_tts.py` | 248 |
| `ABM_VOXCPM_POLL_S` | `2` Intervallo fra due sonde su `/status`. | `voxcpm_tts.py` | 252 |
