...
Inoltre `run_optimization()` persiste `job["gemini_estimate"]` (output di `estimate_book_cost`) prima della chiamata diretta a `run_generation`, così l'audit ha sempre i campi stimati anche quando `/api/generate` non viene chiamato.
