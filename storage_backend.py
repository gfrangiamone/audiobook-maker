...
def download_file(key, local_path):
    ...
    file temporaneo univoco accanto alla destinazione (pid + hex casuale)
    cosi' due download concorrenti verso lo stesso `local_path` non si
    accalcano sullo stesso `.part`, e un download interrotto non lascia un
    file a meta'. Ritorna False se l'oggetto non esiste; solleva su altri
    errori. Il file temporaneo viene sempre ripulito, esito a parte."""
    os.makedirs(os.path.dirname(os.path.abspath(local_path)) or ".", exist_ok=True)
    tmp = f"{local_path}.{os.getpid()}.{uuid.uuid4().hex[:8]}.part"
    try:
        try:
